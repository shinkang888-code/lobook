...
Book Studio는 현재 **Polaris Shell UI + Markdown(Toast UI) 1종 완성** 상태이다. 동일 GitHub 조직에는 **문서 포맷별 오픈소스 20+개**가 이미 포크·개발되어 있으며, **`lofice`가 사실상 통합 검증 허브**로 모든 엔진을 연결해 두었다.

**핵심 설계 원칙:**

1. **`book` = 전자책 제작 허브** (챕터·pageSpec·EPUB export·협업)
2. **`lofice` = 문서 엔진 레퍼런스 구현** (뷰어/편집기 패턴 복제 소스)
...
| **통합 오피스** | [lofice](https://github.com/shinkang888-code/lofice) | `c:\cursor\lofice` | JS/Next.js | **60+ 포맷**, 모든 엔진 통합 레퍼런스 | **P0** 패턴 소스 |
...
| **Office 암호** | [msoffice](https://github.com/shinkang888-code/msoffice) | — | C++ | docx/xlsx/pptx 암·복호화 | **P3** lofice 이식 |
...
## 2. lofice — 통합 레퍼런스 아키텍처

`lofice` README 기준, **이미 Book Studio에 필요한 모든 엔진이 연결·검증**되어 있다. Book 업그레이드는 **lofice에서 proven 패턴을 패키지 단위로 포팅**하는 것이 최단 경로이다.

| lofice 모듈 | 원본 OSS | Book Studio 대응 | 포팅 방식 |
|-------------|----------|------------------|----------|
...
| `LoficeLayout` | Polaris 패턴 | BookEditorShell | ✅ 완료 |
...
| `@eigenpal/docx-editor-react` | **실제 DOCX round-trip** (lofice) | npm, client |
| `html-to-docx` | HTML → DOCX export | npm, server |
| `DocX` (.NET) | 서버 고급 DOCX 생성 | Render sidecar |

**Ribbon Insert:** TipTap Image, Table, Link extensions (lofice `DocxEditor` ribbonMode 참조)

---

### 3.4 HWP/HWPX — hwpreader + hwpx-skill + lofice
...
| **C. 네이티브 편집** | `@rhwp/editor` rhwp-studio | lofice `/hwp-editor` | ✅ (장기) |
...
lofice `services/hwpx-skill-api` 패턴 재사용.
...
| merge/split/rotate | pdf-lib (lofice `stirlingMergePdfs` 등) | PDF 편집 패널 |
...
lofice `PptMasterViewer`, `PptSlideEditor`, `exportSlidesToPptx` 포팅.
...
└── services/                   # ★ optional monorepo
    ├── hwpx-skill-api/         # Python FastAPI (lofice 복제)
    ├── ddddocr-api/
    └── stirling-proxy/
```
...
|--------|------|------|
| Vercel + Supabase | ✅ | 현재 |
| hwpx-skill API | HWPX export 시 | lofice `services/hwpx-skill-api` |
| ddddocr API | PDF 스캔 import | lofice Docker compose |
...
| 1-2 | `@eigenpal/docx-editor-react` Word 탭 옵션 | lofice | WordEditorPanel v2 |
| 1-3 | `@rhwp/core` RhwpCanvasViewer 포팅 | lofice/hwpreader | HwpEditorPanel v2 |
...
| 2-4 | pdf-lib merge/split (lofice port) | Stirling-PDF |
...
2. lofice에서 `RhwpCanvasViewer.tsx` → `book/src/components/editor/hwp/` 포팅
...
| lofice | https://github.com/shinkang888-code/lofice |
