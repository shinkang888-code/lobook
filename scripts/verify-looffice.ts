...
const REQUIRED = [
  ...
  "src/lib/lofice/loficeCatalog.ts",
...
function main() {
  ...
  const installLofice = path.join(ROOT, ".lofice-install.json");
  const installLawygo = path.join(ROOT, ".lawygo-install.json");
  if (fs.existsSync(installLofice)) ok("lofice setup 완료");
  else console.log("  ⚠ npm run setup:lofice 권장");
